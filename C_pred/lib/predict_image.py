...
class PredFolderImg(object):
    ...
    def __init__(self, basicParameters, modelParameters,data_height,data_width):
        ...
        self.ckptFolderPathListNEW=[] #1120509 charley 
    ...
    def run_ckpt(self): #Add by charley 1110830 找出最佳的ckpname 
        ...
        self.ckptFolderPathListNEW=[]
        for i in os.listdir(self.ckptFolderPath):
            try:
                if i.split('.')[1]=='xlsx' or i.split('.')[1]=='pptx' or i.split('.')[1]=='ipynb_checkpoints'or i.split('.')[1]=='json' or i.split('.')[3]=='xlsx#' :
                    ...
                    self.ckptFolderPathListNEW.append(i)
            except:
                self.ckptFolderPathListNEW.append(i) 
                
                
        print(self.ckptFolderPathListNEW)
        for ckptFileName in self.ckptFolderPathListNEW: #os.listdir(self.ckptFolderPath):
            # print(ckptFileName.split('_')[2].strip()) # 列示全部的ckpt值
            k = ckptFileName.split('_')[2].strip() # key trim
            dic[k] = ckptFileName.split('_')[3].strip()  # value trim
            print(k)
        print(dic)  # 列示全部的ckpt值    
        json_object = json.dumps(dic, indent=4)
        print("best solution ckpt-{} !".format(json_object))
        ...
        # for ckptFileName in self.ckptFolderPathListNEW: #os.listdir(self.ckptFolderPath):
            ...
            # k = ckptFileName.split('_')[2].strip() # key trim
            # dic[k] = ckptFileName.split('_')[3].strip()  # value trim
            # want del data code write here 1120616             
    ...
    def run(self):
        ...
        # print(self.ckptFolderPath)
        # print('*****')
        # files = os.listdir(self.ckptFolderPath)
        # for ckptFileName  in files:
        #     print(ckptFileName)
        
        self.ckptFolderPathListNEW=[] # only put 
        for i in os.listdir(self.ckptFolderPath):
            try:
                if i.split('.')[1]=='xlsx' or i.split('.')[1]=='pptx' or i.split('.')[1]=='ipynb_checkpoints'or i.split('.')[1]=='json' or i.split('.')[3]=='xlsx#' :
                    print('Delete_list_name:'+i)   #20230505-yita-增加排除項目 json h5 xlsx#---------------
        ...
        for ckptFileName in self.ckptFolderPathListNEW: #os.listdir(self.ckptFolderPath):
            ...
            if ckptFileName.split('_')[2] != self.ckpname: #找出最佳的 by charley
                ...
                print("delete h5 data in ckpt-{} folder !".format(ckptFileName.split('_')[2].strip()))
