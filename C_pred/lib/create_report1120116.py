...
class CreateAccuracyReport(object):
    ...
    #--------------------------new20220810-yita---------------- 
        ...
        # self.numClass = len(colorDict) #1111003 mark by charley  沈博指示修改
        self.numClass = config["num_classes"]-1  #1111003 沈博指示修改 by charley
        # print(self.numClass)
    ...
    #依原始 的名稱定義的資料: dfSummary
        ...
        # color_list=self.config["color_list"]    #1111003 沈博指示修改 by charley  但有顏色未轉出需再討論
        # for idx, correct_label_name in enumerate(self.config["color_list"]):
    ...
    def SummaryOtherData(self,rowCount):
        ...
        # print(Data_total)
        Data_total_excel=Data_total+'/A_Data_preprocessing/Dentistry.xlsx' # path modify charley 1120116
...
#         self.dfIoUColNames.append('Mean IoU')
    ...
    def setColumnNames(self):
        ...
        for i in range(self.numClass): # 1111003 沈博指示修改 -1 後來取消 
            # self.dfIoUColNames.append('IoU ' + str(i))
            self.dfIoUColNames.append(self.detect_label_dict[i])
        self.dfCountColNames = self.dfIoUColNames.copy()
        self.dfIoUColNames.append('Mean IoU')
