# How to difine
# from lib.check_training import * 
# How to call 
# RJ(save_model_path) 
...
class RJ(keras.callbacks.Callback):   
    ...
    def on_train_end(self, logs=None): #run finished check
        import os
        self.epochs += 1
        epoch=self.epochs
        logs = logs or {}
        ...
        for filename in files:
            ...
            for k, v in top_3_dict:
                ...
                if filename.find(v)>=0:
                    ...
                    break
                elif filename.find(v)< 0 :#compare top 3
                    if mark > 2:
                        print('No刪除') 
                        print(filename)
                        # os.remove(filename)
        ...
        for filename in files:
            ...
            for k, v in top_3_dict:
                ...
                if filename.find(v)>=0:
                    ...
                    # print(str(len(filename.split("_"))))
                    break
                elif filename.find(v)< 0 :#compare top 3 if null 刪除
                    if mark > 2:
                        print('刪除') 
                        print(filename)
                        os.remove(filename)
